...
这里收录 ${project.count} 张中华传统色高清色卡，已按原始 742 色清单完整覆盖。每张色卡包含色名、HEX、RGB、CMYK、配色推荐和气质关键词。现在它不只是素材库，也是一套可交互的中国色工作台：可以查色、试场景、生成色板、浏览配色关系、制作用途卡片，并把喜欢的色卡和整组方案收藏到本机。
...
It contains ${project.count} high-resolution Chinese traditional color cards, aligned with the original 742-color list. Each card preserves the color name, HEX, RGB, CMYK, palette guidance, and mood keywords. The site now goes beyond an archive: you can browse cards, test colors in real layout scenes, generate palettes, explore harmony relationships, build usage cards, and save favorite colors or full schemes locally.
...
${project.count} 枚の高解像度カラーカードを収録し、元の 742 色リストと対応させています。各カードには色名、HEX、RGB、CMYK、配色の参考、雰囲気のキーワードをまとめています。現在のサイトでは、色カード閲覧だけでなく、実際のレイアウトでの試色、配色生成、配色関係の閲覧、用途カード作成、気に入った色や配色のローカル保存もできます。
